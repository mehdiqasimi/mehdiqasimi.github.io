---
layout: homepage
---

## About Me

I am a Doctoral Researcher at Tampere University working in the field of cold spray, surface engineering and materials characterization. 

My research focuses on confined space coating deposition, particle impact behavior, coating characterization and process optimization.

[My CV is here.]({{ site.cv_link }})

## Recent

<div class="publication-entry">

<div class="publication-header">

<div class="publication-title">
Started my doctoral studies at Tampere University, joining the Materials Characterization / Coating Technologies Group.
</div>

</div>

<p class="publication-info">
TTITO Doctoral School, ENS Faculty, 2026
</p>

</div>

<div class="publication-entry">

<div class="publication-header">

<div class="publication-title">
<span class="news-lead">Work from my master's thesis was presented as a talk at ICSC 2026 in Chennai, India:</span><br>
<a href="https://researchportal.tuni.fi/en/activities/in-flight-particles-behavior-and-coating-property-correlation-for/" target="_blank" rel="noopener">
In-Flight Particles Behavior and Coating Property Correlation for IDCS
</a>
<span class="publication-authors">presented by <a href="https://researchportal.tuni.fi/en/persons/yaman-sahu/" target="_blank" rel="noopener">Yaman Sahu</a>, <a href="https://researchportal.tuni.fi/en/persons/heli-koivuluoto/" target="_blank" rel="noopener">Heli Koivuluoto</a>(contributor)</span>
</div>

<details class="publication-details">

<summary>
<span class="show-abstract">Show abstract</span>
<span class="hide-abstract">Hide abstract</span>
</summary>

<div class="abstract-text">

<p>
My master's thesis was presented at ICSC, where Yaman gave a talk on the relationship between in-flight particle behavior and coating properties in inner-diameter cold spray. The talk introduced cold spray and inner-diameter coating applications, described the experimental setup i.e. an ID cold spray gun paired with diagnostic cameras also the briefing the results, how particle velocity, bow-shock effects, deposition thickness, porosity and coating microstructure are shaped by these process parameters.
</p>

</div>

</details>

</div>

<p class="publication-info">
International Cold Spray Conference &amp; Expo 2026, Chennai, India &middot; 25 Jan 2026
</p>

</div>



## Working papers

<div class="publication-entry">
<div class="publication-header">
<div class="publication-title">
<a href="https://urn.fi/URN:NBN:fi:tuni-2025112410843" target="_blank" rel="noopener">
Investigation of In-flight Particles Behavior and Coating Property Correlation for Inner Diameter Cold Spray
</a>
</div>

<details class="publication-details">

<summary>
<span class="show-abstract">Show abstract</span>
<span class="hide-abstract">Hide abstract</span>
</summary>

<div class="abstract-text">

<p>
In master's thesis I investigates inner-diameter cold spray (IDCS) for coating confined geometries, focusing on how gas pressure, temperature, and standoff distance influence in-flight particle behavior and coating properties. Aluminum, copper, and nickel-aluminum feedstocks were studied using particle diagnostics and coating characterization. The results show that particle velocity is governed mainly by gas pressure and standoff distance, while the resulting impact conditions strongly affect coating hardness, porosity, and deposition behavior. Copper produced the densest and most efficient deposits, whereas nickel-aluminum showed limited build-up. Response surface models were also developed to relate process parameters to coating responses, achieving R² values above 0.91 and demonstrating their potential for IDCS process optimization.
</p>

</div>

</details>

</div>

<p class="publication-info">
Master's thesis, Tampere University, 2025
</p>

</div>
